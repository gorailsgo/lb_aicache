= RightScale aiCache Load Balancer Cookbook 

== DESCRIPTION:

This is aiCache load balancer cookbook. 

This cookbook provides recipes for setting up and running an aiCache load
...
This recipe is used by the load balancer servers to reconfigure aiCache and
...
individual configuration file in the aicache.d directory with the instance UUID
as the file name. If the file didn't exist before or if its contents change, the
main aiCache configuration information and all of the files in the aicache.d
directory are concatenated into the aiCache configuration file and aiCache is
restarted.
...
aicache.d directory which has the instance UUID as its file name. If the file
was deleted, the main aiCache configuration information and all of the remaining
files in the aicache.d directory are concatenated into the aiCache configuration
file and aiCache is restarted.
...
update, or delete individual server configuration files in the aicache.d
directory depending on their current status. If any of the files in the
aicache.d directory have been created, changed, or deleted, the main aiCache
configuration and the new set of files in the aicache.d directory are
concatenated into the aiCache configuration file and aiCache is restarted.
